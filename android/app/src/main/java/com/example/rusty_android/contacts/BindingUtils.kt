...
import com.example.rusty_android.convertBalanceToFormatted
import com.example.rusty_android.convertNumericLastSeenToString
...
fun TextView.setLastChatFormatted(item: MeshContact?) {
    item?.let {
        text = convertNumericLastSeenToString(item.lastChatTimeMilli, context.resources)
    }
}
...
fun TextView.setchannelBalanceFormatted(item: MeshContact?) {
    item?.let {
        text = convertBalanceToFormatted(item.ourBalanceMSats, context.resources)
    }
}
