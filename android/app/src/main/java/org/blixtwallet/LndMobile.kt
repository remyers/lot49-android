...
import org.globalmeshlabs.lot49.BuildConfig
...
object LndMobile  {
    ...
    fun sendPongToLndMobileservice(promise: CompletableFuture<HashMap<String, *>>?) {
        ...
        val message =
            Message.obtain(null,
                LndMobileService.MSG_PING, req, 0)
    ...
    fun init(context: Context, promise: CompletableFuture<HashMap<String, *>>) {
        if (!lndMobileServiceBound) {
            ...
            lndMobileServiceConnection =
                LndMobileServiceConnection(req)
        ...
        if (lndMobileServiceBound) {
            ...
            if (lndMobileServiceMessenger != null) {
                try {
                    val message =
                        Message.obtain(null,
                            LndMobileService.MSG_UNREGISTER_CLIENT, req)
                    message.replyTo = messenger
                    lndMobileServiceMessenger!!.send(message)
                } catch (e: RemoteException) {
    ...
    fun checkStatus(promise: CompletableFuture<HashMap<String,*>>?) {
        ...
        val message =
            Message.obtain(null,
                LndMobileService.MSG_CHECKSTATUS, req, 0)
        message.replyTo = messenger
        try {
            lndMobileServiceMessenger!!.send(message)
        } catch (e: RemoteException) {
    ...
    fun startLnd(context: Context, torEnabled: Boolean, promise: CompletableFuture<HashMap<String,*>>?) {
        ...
        val message =
            Message.obtain(null,
                LndMobileService.MSG_START_LND, req, 0)
        message.replyTo = messenger
        val bundle = Bundle()
        var params =
    ...
    fun stopLnd(promise: CompletableFuture<HashMap<String,*>>?) {
        ...
        val message =
            Message.obtain(null,
                LndMobileService.MSG_STOP_LND, req, 0)
        message.replyTo = messenger
        try {
            lndMobileServiceMessenger!!.send(message)
        } catch (e: RemoteException) {
    ...
    */
    ...
    fun unlockWallet(password: String?, promise: CompletableFuture<HashMap<String,*>>?) {
        ...
        val message =
            Message.obtain(null,
                LndMobileService.MSG_UNLOCKWALLET, req, 0)
        message.replyTo = messenger
        val bundle = Bundle()
    ...
    fun genSeed(promise: CompletableFuture<HashMap<String,*>>?) {
        val req = Random().nextInt()
        requests[req] = promise
        val message = Message.obtain(null,
            LndMobileService.MSG_GRPC_COMMAND, req, 0)
        message.replyTo = messenger
        val bundle = Bundle()
    ...
    fun sendStreamCommand(promise: CompletableFuture<HashMap<String,*>>?, method: String, request: MessageLite, streamOnlyOnce: Boolean) {
        ...
        requests[req] = promise
        val message = Message.obtain(null,
            LndMobileService.MSG_GRPC_STREAM_COMMAND, req, 0)
        message.replyTo = messenger
        val bundle = Bundle()
        bundle.putString("method", method)
        bundle.putByteArray("payload", request.toByteArray())
    ...
    fun initWallet(
        ...
        val message =
            Message.obtain(null,
                LndMobileService.MSG_INITWALLET, req, 0)
        message.replyTo = messenger
        val bundle = Bundle()
    ...
    fun copyLndLog(context: Context, promise: CompletableFuture<String>?) {
        checkWriteExternalStoragePermission(
            context,
            object :
                RequestWriteExternalStoragePermissionCallback {
                override fun success(value: Any?) {
                    if (value == "granted") {
                        val lndLogFile =
                            copyLndLogFile(context)
                        if (lndLogFile != null) {
