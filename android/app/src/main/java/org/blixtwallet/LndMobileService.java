...
public class LndMobileService extends Service {
    ...
    Map<String, SendStream> streamsStarted = new HashMap<>();
        ...
        public void handleMessage(Message msg) {
            ...
            switch (msg.what) {
                ...
                case MSG_GRPC_STREAM_COMMAND:
                    ...
                    if (m == null) {
                        ...
                        if (m == null) {
                            ...
                            for (Method n : methods) {
                                ...
                                if (isStream(n)) {
                                    HyperLog.d(TAG, "found streaming method " + name);
                                }
                                else {
                                    HyperLog.d(TAG, "found normal method " + name);
                                }
                            }
                    ...
                    try {
                        if (msg.what == MSG_GRPC_COMMAND) {
                            m.invoke(null,
                                    b,
                                    new LndCallback(msg.replyTo, method, request)
                            );
                        } else if (msg.what == MSG_GRPC_STREAM_COMMAND) {
                            m.invoke(null,
                                    b,
                        ...
                        /*
                            ...
                            if (streamOnlyOnce) {
                                if (streamsStarted.containsKey(method)) {
                            ...
                            Bundle sendBundle = new Bundle();
                            sendBundle.putString("method", method);
                            lndmobile.SendStream stream;
                            if (streamsStarted.containsKey(method)) {
                                stream = streamsStarted.get(method);
                            } else {
                                stream = (SendStream) m.invoke(null,
                                        new LndStreamCallback(msg.replyTo, method));

                                if (stream != null) {
                                    stream.send(b);
                                    if (!streamOnlyOnce) {
                                        streamsStarted.put(method, stream);
                                    }
                                    else {
                                        stream.stop();
                                    }
                                } else {
                                    bundle.putString("error_desc", "Method returned null stream, could not send request.");
                                    Log.e(TAG, "lndmobile streaming method " + method + " returned null, could not send request.");
                                }
                            }
                ...
                case MSG_CHECKSTATUS:
                    ...
                    flags += LndMobile.LndStatus.SERVICE_BOUND.getFlag();

                    if (lndStarted) {
                        flags += LndMobile.LndStatus.PROCESS_STARTED.getFlag();
                    }

                    if (walletUnlocked) {
                        flags += LndMobile.LndStatus.WALLET_UNLOCKED.getFlag();
                    }
